...
* `uuid` - (Optional) A stable UUID for the job. When supplied, it is preserved as the
  job's Rundeck UUID across applies and clusters, which keeps `<jobref>` references by
  UUID stable and avoids duplicate jobs on re-create. If omitted, Rundeck generates a
  UUID and it is recorded in state. Changing a configured `uuid` forces the job to be
  destroyed and recreated. Must be a canonical lowercase UUID (for example
...
* `id` - A unique identifier for the job. Equal to the job's UUID.

* `uuid` - The job's Rundeck UUID. Equal to `id`; set to the supplied `uuid` when one
  was given, otherwise the UUID generated by Rundeck.
