...
func commandFromMap(commandMap map[string]interface{}) (*rundeck.JobCommand, error) {
	...
	if errorConfigI, present := commandMap["errorhandler"]; present {
		...
		if len(errorConfigI) > 0 {
			...
			errorHandler, err := commandFromMap(errorConfigMap)
			if err != nil {
				return nil, err
			}
...
func jobNotificationFromConfig(configI map[string]interface{}, handle string) (*rundeck.Notification, error) {
	...
	if len(notificationConfigI) > 0 {
		notification, err := notificationFromMap(notificationConfigI[0].(map[string]interface{}))
		if err != nil {
			return nil, err
		}
		return notification, nil
...
func jobFromResourceData(d *schema.ResourceData) (*rundeck.JobDetail, error) {
	...
	if len(notificationConfigI) > 0 {
		...
		onFailure, err := jobNotificationFromConfig(notificationConfigI, "onfailure")
		if err != nil {
			return nil, err
		}

		onStart, err := jobNotificationFromConfig(notificationConfigI, "onstart")
		if err != nil {
			return nil, err
		}

		onSuccess, err := jobNotificationFromConfig(notificationConfigI, "onsuccess")
		if err != nil {
			return nil, err
		}
	...
	for _, commandI := range commandConfigs {
		command, err := commandFromMap(commandI.(map[string]interface{}))
		if err != nil {
			return nil, err
		}
...
func commandToMap(command rundeck.JobCommand) (*map[string]interface{}, error) {
	...
	if command.ErrorHandler != nil {
		errorHandler, err := commandToMap(*command.ErrorHandler)
		if err != nil {
			return nil, err
		}
...
func jobToResourceData(job *rundeck.JobDetail, d *schema.ResourceData) error {
	...
	if job.Notification != nil {
		...
		if job.Notification.OnFailure != nil {
			notificationMap, err := notificationToMap(job.Notification.OnFailure)
			if err != nil {
				return err
			}
		...
		if job.Notification.OnStart != nil {
			notificationMap, err := notificationToMap(job.Notification.OnStart)
			if err != nil {
				return err
			}
		...
		if job.Notification.OnSuccess != nil {
			notificationMap, err := notificationToMap(job.Notification.OnSuccess)
			if err != nil {
				return err
			}
	...
	if job.CommandSequence != nil {
		...
		for _, command := range job.CommandSequence.Commands {
			commandConfigI, err := commandToMap(command)
			if err != nil {
				return err
			}
